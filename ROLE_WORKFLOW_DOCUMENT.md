...
| Workflow Batches | permission **38** directly (`PermissionService::has`, mirrors legacy's Batches gate, now also granted to Account/6 — see §11) |
...
2. **Account role (6) formalized.** Legacy has it as a bare `USER_TYPE=='6'` gate with no `user_type` lookup row and zero `role_priviledge` rows (2 real users only). This session added `user_type` row 6 ("Account"), made it selectable in Create/Edit User, and granted it: (a) role-membership in the `applications.view`/`applications.submit`/`applications.documents.view`/`workflow.view`/`workflow.action` abilities (reusing `DataScopeService`'s pre-existing but previously-unreachable `workflow_stage=accounts`/finance-status scoping — written in an earlier session, never wired to any ability until now), and (b) `role_priviledge` permission 38 for menu parity on the Workflow Batches screen. **Rationale:** "apply permissions exactly as legacy CI3" cannot be literally satisfied since legacy grants Account *zero* permissions; the closest faithful interpretation is to let Account reach the one screen (Workflow Batches, which Laravel already consolidates IC/DU/Circle batch actions AND the payment-result/forward/remove actions into) needed to perform its two real legacy actions (`remove`/`forward`), scoped only to the finance-stage subset of applications it could see in legacy.
