...
| Payment pipeline: `proceedpayment`/`finishpayment` (AXIS bank file) + `uploadCsv` (UTR reconciliation), §1.9/§1.10/§11 | `ScholarshipService::createPaymentBatch()` writes the pipe-delimited AXIS `.txt` outbound file (`generateAxisPaymentFile()`) to a configurable path (`config('axis_payment.output_path')`, env `AXIS_PAYMENT_OUTPUT_PATH`; legacy hardcoded `/data/AxisSnorkel/In/`). `App\Console\Commands\ReconcileAxisReverseFeed` (scheduled every 15 minutes, `routes/console.php`) replaces manual `uploadCsv()` with an automated reverse-feed scan — **not** `uploadCsv()`'s CSV format (that was this document's initial, unverified assumption; corrected against a real production reverse-feed sample supplied directly by the user). The real feed is `^`-delimited, no header row, no file extension, named `axis_reversefeed_*`, and is **shared across CGMFPFED modules** (e.g. Beema uses the same feed/format with its own reference values) — field 0/9 is the payment reference (matched against `application_number`; a non-matching reference, e.g. Beema's, is silently skipped, not an error), field 6 is `SUCCESS`/a rejection code, field 7 is `"Success--{UTR}"` on success or a human-readable failure reason otherwise, field 11 is the bank's own transaction reference (present on both outcomes, used as `payment_reference_id`) | Migrated, with a corrected reconciliation model | Deliberate departures from legacy, from explicit business decisions in this session (not inferred): (1) the AXIS file is generated automatically at payment-batch-submission time rather than via a separate manual "Finish Payment" confirmation click; (2) reconciliation is a scheduled/automatic folder scan, not a manual CSV upload screen — legacy's `upload_csv.php` UI was intentionally not ported. Only applications currently at `PaymentBatchSubmitted` (99) are reconciled, so a stale/duplicate feed row can't silently overwrite an already-settled application. Legacy's via-CCF lineage-loss bug (§1.10) doesn't apply since via-CCF statuses are already unreachable in Laravel |
| Account's `forward()`/`remove()` (15/16↔28, §1.8) and HQ's failed-payment reprocessing (17→15, §2) | `ScholarshipService::nextStatus()` now has explicit `forward` (Account, 15→28), `remove` (Account, 28→15 undo), and `retry` (Account/HQ, 17→15) actions, each enforced by `ScholarshipApplicationPolicy::act()` — previously these were reachable only as an unrestricted `recommend` action available to every workflow role | Migrated, with role enforcement added | Closes a real gap: before this change `workflow.action` granted every non-VLE role identical access to every transition regardless of stage: `ScholarshipApplicationPolicy::act()` now requires the acting role to match legacy's per-stage responsible role (Samiti/IC/District Union/HQ/Account) before any transition executes, and `recordPaymentResult()` is now HQ-only. This is the fix for the task instruction "do not replicate the legacy Visitor.php authorization bug" — Laravel's Gate/Policy layer now does what legacy's inline `USER_TYPE==` checks did, not what `Visitor.php` only appeared to do |
| VLE deletion (`Scholarship::delete()`, §1.11, statuses `0,1,21-26`) | `ScholarshipService::deleteDraft()` + `ScholarshipController::destroy()`, gated by `ScholarshipApplicationPolicy::delete()` (VLE-only, own application, same status allow-list) | Migrated, as a soft delete | Legacy hard-deletes the row; Laravel soft-deletes (`SoftDeletes` already on `ScholarshipApplication`) so the audit trail this task requires ("every workflow action must create an audit trail") remains intact for the delete action itself — a deliberate, minor improvement, not a behaviour gap |
| IC's live per-application amount override (`verify_application.php`'s `updateamount()` AJAX, `scheme_helper.php::getAmount()`, §1.3/§11) | `ScholarshipService::amountOptionsForScheme()` (exact same per-scheme fixed-amount lists as legacy's `getAmount()`) + `modifyAmount()`, invoked only from `createIcBatch()` (IC batch/MoM creation) via `workflow.ic-batches.store`'s `amounts[]` payload, gated `workflow.ic-batch` (role 4/IC only — HQ cannot call this route) | Migrated | Legacy's override is a free-standing AJAX call available throughout the IC verification screen; Laravel folds it into IC batch (MoM) creation specifically, per an explicit business decision in this session. Every override is audited (`amount_modified` action, capturing the standard/automatic amount vs. the IC-selected amount) — legacy's `update_amount` table equivalent |
