...
### 6.5 Permission gates (`hooks/Visitor.php:42-51`)
```
user::add_user   requires role_priviledge permission_id == 1
user::edit_user  requires role_priviledge permission_id == 2
user::vle_profile is unconditionally blocked for any non-VLE session
```
In production `role_priviledge` data, only role 1 (Super Admin) holds both permissions 1 and 2 (`scholarship.sql:1062`, rows `(490,1,1)` and `(491,1,2)`) — no other role can create or edit users through this screen in the current permission grants.
...
8. **`users.status` enum's third value.** The schema is `enum('0','1','2')` (`scholarship.sql`), but no code path referencing `status=='2'` was found in `User.php`. **NOT VERIFIED** what status 2 means for a user record, or whether any other file (not yet examined) handles it.
